...
max_step_x = 0.07  # 最大正向平移沿X（米）0.065
max_step_y = 0.142  # 沿Y方向的绝对最大横向平移（米）0.145
max_step_theta = 0.2  # 绕Z轴绝对最大旋转（弧度）0.2
max_step_frequency = 0.75  # 最大步进频率（标准化，无单位）0.65
step_height = 0.024  # Z轴高程（米）0.021
...
def straight_move(x, y=0.0, z=0.0):
    ...
    motion.moveTo(x, y - 0.015, z - 0.01,
                  [["MaxStepX", max_step_x],
                   ["MaxStepY", max_step_y],
...
def robot_socket_client(port=7744, host=""):
    """
    接收端
...
def analyze2():
    ...
    if message[1] is 0.0 and message[2] is 0.0:
        ...
        if message[2] < 20.0 or message[1] > 40.0:
            Z = Z - 0.1
            if message[1] > 200 or message[2] < 10.0:
                Z = Z - 0.1
                if message[2] == 0.0:
                    Z = Z - 0.1
            return
        ...
        if message[1] > message[2] + 40 or message[1] > message[2] * 1.5:
            Y = Y - 0.04
            if message[1] > message[2] + 120 or message[1] > message[2] * 2:
                Y = Y - 0.04
                Z = Z - 0.1
                if message[1] > message[2] + 250:
                    Y = Y - 0.04
                    Z = Z - 0.1
        ...
        if message[2] > message[1] + 80 or message[2] > message[1] * 2:
            Y = Y + 0.02
            Z = Z + 0.1
            if message[2] > message[1] + 180 or message[2] > message[1] * 2.5:
                Y = Y + 0.02
                Z = Z + 0.1
                if message[2] > message[1] + 280:
                    Y = Y + 0.02
                    Z = Z + 0.1
...
def analyze():
    ...
    Y = 0.0
    Z = 0.0
    if message[1] < 20 and message[2] < 20:
        return
    if message[1] > message[2]:
        Y = Y - 0.1
        if message[1] - message[2] < 80:
            Z = 0.0
        elif message[1] - message[2] < 150:
            Z = -0.1
        elif message[1] - message[2] < 200:
            Z = -0.2
        elif message[1] - message[2] < 300:
            Z = -0.5
        elif message[1] - message[2] < 440:
            Z = -0.4
        elif message[1] - message[2] < 470:
            Z = -0.5
        elif message[1] - message[2] < 500:
            Z = -0.3
        elif message[1] - message[2] < 560:
            Z = -0.4
        else:
            Z = -0.6
    if message[2] > message[1]:
        Y = Y + 0.1
        if message[2] - message[1] < 80:
            Z = 0.0
        elif message[2] - message[1] < 150:
            Z = 0.1
        elif message[2] - message[1] < 200:
            Z = 0.2
        elif message[2] - message[1] < 300:
            Z = 0.5
        elif message[2] - message[1] < 440:
            Z = 0.4
        elif message[2] - message[1] < 470:
            Z = 0.5
        elif message[2] - message[1] < 500:
            Z = 0.3
        elif message[2] - message[1] < 560:
            Z = 0.4
        else:
            Z = 0.6
    Y = Y + dev_y
...
def main():
    ...
    for i in range(5):
        ...
        # if i != 4:
        # time.sleep(1)
        straight_move(x=1.5, y=Y, z=-0.06)
        # added_thread.join()
